...
async function submitForm(event) {
  ...
  try {
    ...
    event.target.reset();
    switchPage('list'); // Go to planning
  } catch (error) {
...
function renderLists() {
  ...
  if (allRegistrations.length === 0) {
    const emptyMsg = '<p style="text-align:center; padding:2rem; color:rgba(255,255,255,0.2)">Aucune donnée enregistrée</p>';
  ...
  return `
    <div class="entry-card">
      <div class="entry-info">
        ...
        <span class="entry-dates">${label} : <b>${formatDate(dateToShow)}</b><br>Séjour : ${formatDate(data.arrival)} - ${formatDate(data.departure)}</span>
      </div>
